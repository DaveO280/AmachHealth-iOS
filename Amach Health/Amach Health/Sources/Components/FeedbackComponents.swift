        ...
        VStack(spacing: AmachSpacing.xl) {
            ...
            VStack(spacing: AmachSpacing.sm) {
                Button(confirmLabel) { onConfirm() }
                    .buttonStyle(
                        isDestructive
                            ? AnyButtonStyle(style: AmachDestructiveButtonStyle())
                            : AnyButtonStyle(style: AmachPrimaryButtonStyle())
                    )
...
private struct AnyButtonStyle: ButtonStyle {
    ...
    init<S: ButtonStyle>(style: S) {
        _body = { config in AnyView(style.makeBody(configuration: config)) }
    }
